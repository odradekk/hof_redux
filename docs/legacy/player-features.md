...
| tactics.patterns | 正式 | 角色页 `ChangePattern`/`PatternMemo`/`AddNewPattern`/`DeletePattern` | `CharStatProcess` 445-515；`CharStatShow` 850-892；`class.char.php::AddPattern` 299-313、`DeletePattern` 315-329、`ChangePatternMemo` 345-348、`PatternSave` 1255-1259；`data.judge_setup.php` | 每角色持久化当前和备用两套有序条件-动作方案，可修改数量阈值、交换主/备方案、增删规则行；可“设置并测试”。备用初始可未配置，并非任意多套方案 | 角色与编队；战斗引擎 | 条件与动作全集、优先顺序、数量解析、规则行数上限、主备交换与未配置状态、战斗选择结果 |
